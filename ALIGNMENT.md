...
| Parameter constraint solver (math boundaries) | **ALIGNED, with a corrected count** | closed-form checks plus one geometry build, ~0.11 ms, reasons reported (`grammar.check`). The "30+" this row used to claim was wrong and so was the plan's "49": the 2026-08-05 audit measured **9 live** constraints — 28 emitted, of which 15 are bound checks the optimiser cannot violate and 4 more are tautologies inside the declared bounds (0 hits in 400 000 in-bounds samples). Gap E4. **RE-MEASURED 2026-08-13 on the plate-P1/P2 genome: 8 live relations plus 2 named REFUSALS** (`sac.target`, `section.solve` — a design target the shape family cannot reach, and a station whose area its draft and deadrise cannot enclose; both raised as `GeometryError` and reported by name, not as booleans), over 16 bound checks. `chine.height` was replaced by the tighter `chine.submerged` and `transom.chine` deleted on gap E4's own rule after firing on 0 of 40 000. `tests/test_constraints_honest.py::test_every_relational_l0_constraint_can_actually_fire` pins the count and the hit table. |
