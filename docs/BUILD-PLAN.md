...
| PV-4 **LANDED 2026-08-28 (Gate MULTI-CHINE)** — the section law is a KNUCKLE LIST: `Hull._topside_chain` walks the interior topside vertices and the clip's k=0 / k=1 members ARE the legacy and Phase-3 shapes, expression for expression, so every pre-Phase-5 fence holds at exactly 0.0 rather than at a tolerance. Genes `ch2_z` / `ch2_y`, appended under POST_HOC_DEFAULTS and pinned at (0, 0) in the DRAW box. WHAT REMAINS OF THE ROW: the `hull_ast.Pin(roundness)` retirement, which needs the UNROLLER to accept a two-chine developable panel set — the section law no longer blocks it. | **A multi-chine section law.** The grammar reaches **two of the five standard body plans** and has ONE chine; the two it cannot draw are the double-chine forms (`docs/research/HULL-FORM-RULES.md`). Multi-chine is the plywood answer to a round bilge, and it is the honest replacement for the `roundness = 0` pin now applied to sheet-built typologies — that pin is a STOPGAP correct for the unroller we have, **not a principle** | **new Gate 13C** | the grammar draws all five body plans, and `hull_ast.Pin(roundness)` is retired against a section law rather than against a tolerance |
