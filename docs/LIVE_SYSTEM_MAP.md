...
GENOME      navalai/grammar.py::vector/named/check(vessel=)     ← the one parameterisation + L0 gate
   sources: evaluate.py::sample_valid │ optimize.py::pareto_front │ generative.py::make_generator
        ▼
...
| EXPERIMENT | agents.py + hull_ast.py (the Phase-2 agentic island, test-only), policy/ (governance kernel, unwired), pipeline.py + latent front (lifecycle FSM / latent search, unwired), arrangement.py (tier-E, awaiting R2.4) |
