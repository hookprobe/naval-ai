# RESEARCH RECORD — APSE: similitude, fidelity, cost

> **Role: RESEARCH / EVIDENCE.** The measured cost model behind the L2/L3
> tiers, the similitude rules, and the refutation that made geometric scale
> stop being a search variable. No plan, no gate status — the plan is
> `docs/BUILD-PLAN.md`, status is `python -m navalai.gates`.
>
> APSE ships as five modules under one gate (`tests/test_stageG.py`):
>
> | module | owns |
> |---|---|
...
> | `navalai/fidelity.py` | the cost knob: mesh density → cells, steps, wall-clock, RAM; fail-fast admission |
...
>
> **Provenance caveat (LESSONS defect class 5):** the cost constants in §4 were
> measured on `runs/kcs_sym` and `runs/kcs_iso`. Both directories were still
> present on this Mac on 2026-08-11 — but `CELL_STEP_S`, `COURANT_EFFICIENCY`
> and `REFINE_FACTOR` each name their source run in place, and a re-measure is
> cheap. Do not carry them forward silently if those runs are ever purged.

---

## 1 · The finding that reshaped the brief: geometric scale buys no CPU
...
|---|---:|---:|---:|
| 1:100 of the ship | 2.3000 | 2.492e+06 | **13 608** |
| KCS model 1:31.6 | 7.2786 | 1.403e+07 | **13 608** |
| KCS ship 1:1 | 230.0000 | 2.492e+09 | **13 608** |
...
number of flow-throughs (`T ~ L/U`), so the **step count is scale-invariant
too**.
...
Guarded by `test_geometric_scale_buys_no_cpu` and
`test_cost_does_not_depend_on_hull_size`.

---

## 2 · Similitude

`Condition(lwl, speed, rho, nu, sigma_surface, g)` → `fn`, `re`, `we`,
`wavelength`.
...
Accelerations are **unchanged** — the classic trap. `rho_ratio` is applied only
to mass/force/moment/pressure/power (a fresh-water tank result for a seawater
ship is 2.7% light in force, and nothing in velocity).

`scale_effects(ship, model)` computes every group on **both conditions as
given** — so a tank that ran 2% off the target speed, or in fresh water for a
seawater ship, shows up as a real mismatch rather than a clean λ^-1.5 that was
never achieved. It returns `admissible=False` with reasons when:

- `Re_model < 1e6` — below the accepted floor for a resistance model
  (ITTC 7.5-02-02-01); substantially laminar, and **no extrapolation undoes
  that**;
- `Re_model < 5e6` — transition is set by the turbulence-stimulation device,
  not the hull; a CFD model at this Re has no such device;
...
restated — the same single-source rule `limits.py` exists to enforce.
...
identity and the assumption is never invoked**
(`test_extrapolation_is_the_identity_at_full_scale`).
...
constant: it is well above the ~1.1 the Tokyo-2015 groups use for this hull, and
at that value almost no wave-making is left at a Froude number where KCS
demonstrably makes waves. The likelier reading is that it absorbed a **friction
shortfall** — which was the known state of the mesh at the time (y+ median 2475,
32% layer coverage on the old grid). The check does not refuse it; it refuses to
let it pass unremarked. **Unreconciled, and still open.**

---

## 4 · Fidelity and cost — every constant measured from `runs/` logs

`runs/kcs_sym` — 241 946 cells, symmetric, np=10, one unbroken ExecutionTime
segment, 3 145 steps to t = 13.739 s:
...
np=15 → 1.204× slower than np=10
(`test_parallel_correction_reproduces_the_measured_np_sweep`).
...
### The GCI triplet costs ~21×, not ~12× — a correction to a budget in CLAUDE.md
...
cell ratio is used as a time estimate. **This correction has never been merged
back into `CLAUDE.md`, so a session reads the wrong number first.**
...
stops the cheapness search running away to free); tank seiche on the wave
period; > 6 h without resumability (`pmset -g log`, Thermal Emergency Sleep
2026-08-04).
...
### Tank length resonance is Froude-similar and survives scaling

```
T_seiche = 2 L_tank / √(g h)
```

No hull term — it is a property of the **box**, so no hull or mesh change
removes it. With the depth floor binding (h = 0.6 L) and the domain at 4.5 L:

```
T_seiche / √(L/g) = 2·4.5/√0.6 = 11.62        T_wave / √(L/g) = 2π Fn
```

Their **ratio is a function of Froude number alone**, so it is Froude-similar
and **survives scaling exactly**
(`test_seiche_is_froude_similar_and_survives_scaling`). Shrinking the model
carries the resonance along into every case, unchanged.
...
frequency contamination. But that run had reached only t = 13.7 s = **1.4 seiche
periods**, so any force average from it crosses an incompletely damped tank
oscillation. Those are two different findings and the check separates them.

*(Note the relationship to `research/CFD.md` §2: at 3.40 flow-throughs on the
current mesh family there is no measurable coherent oscillation of any period.
The seiche check remains a correct admission guard on the SPECTRUM; it is not
evidence that a tank mode is contaminating a given run.)*

### The stated budget and the wave-resolution bar cannot both hold

Two of this project's own stated constraints conflict, and the conflict is
pinned by
`test_stated_budget_and_the_wave_resolution_bar_are_unsatisfiable_together`:
...
### Consequence for a GCI triplet

Building **upward** from density 1.0 (`--anchor coarse`): 3.24 + 13.58 + 51.89
= **68.7 h ≈ 2.9 days**, which is 21.2× the coarse grid — confirming the cost
ratios above rather than the 3×/8× cell ratios.
...
## 5 · The planner: information gain per CPU-second
...
- **"Do I actually need CFD?" is answered by arithmetic.** No CFD tier informs
  GM, so CFD's gain on initial stability is exactly zero and it is never
  selected — at any budget (`test_stability_question_never_selects_cfd`).

Refusals are **returned, not raised**, so a rejected option keeps its cost and
reason for comparison instead of vanishing. `QUESTION_QUANTITIES` maps an
engineering question to the quantities it needs; the tier is then *discovered*
from the uncertainty arithmetic rather than hard-coded.

**It deliberately does not choose a geometric scale** — §1 shows scale has no
effect on the objective, so searching it would return an arbitrary answer
...
## 6 · Open, and honestly recorded

- **RAM per cell is assumed, never measured.** Everything else here is measured.
- **The √2 step does not refine z.**
  `test_z_bands_freeze_between_coarse_and_medium_MEASURED_DEFECT` pins a real
  defect found by this suite: the largest-remainder apportionment puts
  `nz_hull = 2` at *both* density 0.7071 and 1.0, so the interface cell height —
  and therefore dt — is **identical on the coarse and medium grids**. A GCI built
  on that pair bounds x–y discretisation only. At 0.7071 the two core z-bands are
  also 2 and 1, where CLAUDE.md records they must be equal.
...
- The extrapolation's `C_R` Re-invariance band (10%) is a stand-in until
  full-scale CFD measures it.

## Extension points

- New similarity law: add to `Similarity` and a group to `scale_effects`.
- New tier: add rows to `TIER_SIGMA_REL` and a cost to `TIER_COST_S`; the
  planner picks it up with no other change.
- New engineering question: one row in `QUESTION_QUANTITIES`.
- Re-calibrate cost: `CELL_STEP_S`, `COURANT_EFFICIENCY`, `REFINE_FACTOR` are
  the three measured constants, each with the run it came from named in place.
