...
def test_two_demihulls_are_two_shells(ref_hull):
    ...
    MEASURED on `mid_params` at the 6000 kg mission: shell area 46.707 m^2 per
    demihull, `structure_kg` 1268.385 -> 2129.074. NOT exactly 2x, and it must
    not be — `weight_budget` also carries the deck and the fit-out, and only
    the SHELL is built twice. What is STILL not counted is carried on the
    evaluation rather than left to be found.
    """
    ...
    assert mono.weights.structure_kg == pytest.approx(1268.385, abs=1e-2)
    assert cat.weights.structure_kg == pytest.approx(2129.074, abs=1e-2)
