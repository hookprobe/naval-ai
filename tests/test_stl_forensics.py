...
def test_stl_resolution_is_clamped_for_every_hull_in_the_batch():
    ...
    # about, while asserting nothing STL.md relies on. What STL.md relies on is
    # that the count is a function of (nx, nz) and the stem, and NOT of LWL, so
    # that is what is asserted: a spread under 1% across a 2.7x range of hull
    # length, and a rank correlation with LWL that is not the near-perfect one
    # a size-driven count would show.
    ...
    assert abs(rho) < 0.9, f"triangle count now tracks LWL (Spearman {rho:.3f})"
