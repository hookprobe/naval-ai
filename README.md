...
| Gate PREFLIGHT | before a solve is bought: theory answers what it can, a surface already solved is recognised, the family band speaks, an impulsive start above the measured Fn ceiling is refused — and after it, Kelvin validates and an A/B is only attributable when the meshes match | `tests/test_preflight.py` (8 tests) |
| Gate CFD-KB | the CFD anchor book: every record honest about settledness and GCI, unsettled records never support a prediction, family bands refuse outside measured Fn support, and the hb19 L1 anchor carries its sigma and basis | `tests/test_cfd_kb.py` (10 tests) |
| Gate SPLIT | the split stern: split genes are no-ops at zero, the SAC stays the net contract, the wall is vertical at y_split, the waterplane integrals subtract the hole, and split + tunnel on one station is refused by name | `tests/test_split.py` (8 tests) |
...
| Gate 0F | the hull-form library: bands ordered, every band carries its basis, no family contradicts its own Froude regime | `tests/test_formlib.py` (50 tests) |
