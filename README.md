...
| Gate DWL | the design waterline: dwl = 0 is bit-identical legacy, the SAC stays the displacement contract at every dwl, the barge passes the general critic, and the requested-vs-achieved gap is a receipt (Hull.dwl_deviation) | `tests/test_dwl.py` (8 tests) |
